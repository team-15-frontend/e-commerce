...
export default function FiltersForm({ className }) {
  ...
  const { category, minprice, maxprice } = urlValues
  ...
  useEffect(() => {
    ...
    trigger(['category'])
  }, [category, minprice, maxprice, urlValues, trigger])
    ...
    >
      <div className="flex flex-col gap-1">
        ...
        <FormField
          ...
          value=""
          register={register}
          parentClassName="flex-row-reverse justify-end gap-4 [[type='radio']]:cursor-pointer"
        />
        ...
        {['electronics', 'phones', 'fashion', 'home', 'beauty', 'sports'].map((category) => (
          <FormField
            ...
            parentClassName="flex-row-reverse justify-end gap-4"
          />
        ))}
      </div>
